...
- Hunter may execute the trusted source bootstrap command from the host message exactly once and unchanged. It is the only permitted network acquisition. It installs one integrity-pinned public source package and a checksum-verified Node runtime without credentials. Do not clone repositories, download any other archive, install any other package or inspect dependency repositories.
- If the trusted bootstrap fails, stop unresolved with its exit status and sanitized error. Never improvise another download, package version, mirror or install command.
