...
- Hunter may execute the trusted source bootstrap command from the host message exactly once and unchanged. It is the only permitted network acquisition. The checksum-pinned script installs a checksum-verified Node runtime and the complete immutable npm lock closure. It verifies every registry package integrity, audits npm signatures, verifies the SLSA package provenance and proves the vulnerable source blob is identical at the issue and provenance commits. Do not clone repositories, inspect dependency repositories or acquire anything outside that exact command.
...
- Hunter may use at most two sandbox commands: the exact bootstrap followed by one deterministic reproduction command. Surgeon and Insurance may use at most one sandbox command each. After bootstrap, reuse its persistent source directory and pinned Node binary. Never reinstall.
...
- Before source-level execution, run the trusted source bootstrap unchanged. Run one bounded reproduction command against that installed artifact and record the command, configured timeout, elapsed boundary and contrasting responsive condition.
